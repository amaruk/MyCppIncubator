 ...
 * thread：线程类，this_thread命名空间
 * mutex：互斥量相关类，mutex，lock_guard，unique_lock
 * condition_variable：条件变量相关，condition_variable和condition_variable_any
 * future：Provider类，promise，package_task。future类future，shared_future
 */
...
void futureTest(void)
{
  ...
  // future表示一个可能还没有实际完成的异步任务的结果
  // future的状态通过future_status来获取，有三种：
  // * deferred：异步操作还没开始
  // * ready: 异步操作已经完成
  // * timeout：异步操作超时
  // 获取future结果有三种方式：
  ...
  promise<int> promiseInt; // 生成类型为int的promise对象，保存了int类型的共享状态
  future<int> futureInt = promiseInt.get_future(); // 获取promise共享状态关联的future对象
  thread threadFuture1(threadPromise, std::ref(futureInt)); // 讲future交给另一个线程
  cout << "main sleeps 5 seconds." << endl;
  sleep_for(seconds(5));
  cout << "main wakes up." << endl;
  promiseInt.set_value(123); // 设置共享状态的值，此后promise的共享状态变为ready
  threadFuture1.join();

  cout << endl << "----PACKAGED_TASK----" << endl;
  packaged_task<void(void)> pkgTask(threadPackagedTask); //packaged_task保存了可调用对象
  ...
  future<void> futureAsync = async(asyncMain, 3); // 默认policy，相当于std::launch::async
  ...
  //futureDeferred.get(); // 调用get或wait来启动async
  futureDeferred.wait(); // 调用get或wait来启动async
